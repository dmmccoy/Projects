  ...
  channels() 
{
  ...
  music = Mix_LoadMUS("sound/gourmetRace.mp3");
  // Need to install midi to play the following:
  // music = Mix_LoadMUS("sound/ballad2.mid");
  if (!music) throw string("Couldn't load gourmetRace.mp3")+Mix_GetError();

  startMusic();

  sounds.push_back( Mix_LoadWAV("sound/kirbyDeath.wav") );
//  sounds.push_back( Mix_LoadMUS("sound/kirby-death-sound.mp3") );
//  sounds.push_back( Mix_LoadMUS("sound/victory.mp3") );
